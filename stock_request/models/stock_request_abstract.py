...
class StockRequest(models.AbstractModel):
    ...
    stock_reference_id = fields.Many2one(
        ...
        "stock reference group. If none is given, the moves generated by "
    ...
    def _compute_allowed_uom_ids(self):
        """Compute allowed UoMs based on product's UoM configuration.
        
        In Odoo 19, UoM categories were removed. UoMs are now related via
        relative_uom_id parent-child relationships. This computes the list
        of valid UoMs for the product.
        """
        for record in self:
            if record.product_id:
                # Get the product's default UoM and any additional UoMs
                record.allowed_uom_ids = (
                    record.product_id.uom_id | record.product_id.uom_ids
                )
            else:
                record.allowed_uom_ids = self.env["uom.uom"]
