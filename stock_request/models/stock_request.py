...
class StockRequest(models.Model):
    ...
    @api.constrains("order_id", "stock_reference_id")
    def check_order_stock_reference(self):
        for stock_request in self:
            if (
                stock_request.order_id
                and stock_request.order_id.stock_reference_id
                != stock_request.stock_reference_id
            ):
                raise ValidationError(_("Stock reference must be equal to the order"))
    ...
    def _prepare_procurement_values(self, reference_id=False):
        ...
        elif self.stock_reference_id:
            reference_ids = self.stock_reference_id
        return {
    ...
    def _action_launch_procurement_rule(self):
        ...
        for request in self:
            ...
            values = request._prepare_procurement_values(
                reference_id=request.stock_reference_id
            )
            try:
                procurements = []
                procurements.append(
