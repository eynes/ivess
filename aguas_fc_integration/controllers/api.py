...
class AguasFCController(http.Controller):
    def _check_api_key(self):
        ...
        expected_key = (
            request.env["ir.config_parameter"].sudo().get_param("aguas_fc.api_key")
        )
    ...
    def actualizar_entrada(self, **kwargs):
        error = self._check_api_key()
        if error:
            return error
        ...
        if "equipos" not in data:
            return {
                "success": False,
                "error": "Campos requeridos faltantes: ['equipos']",
            }

        picking_id = data.get("picking_id")
        if not picking_id:
            error = self._check_required(data, ["idreparto", "fecha"])
            if error:
                return error
        ...
        try:
            return (
                request.env["aguas.fc.intake"]
                .sudo()
                .process_actualizacion(
                    equipos=self._as_list(data["equipos"]),
                    picking_id=picking_id,
                    idreparto=data.get("idreparto"),
